...
window.addEventListener('DOMContentLoaded', () => {
    ...
    async function refreshBBoxes() {
        ...
        if (boxes.length > 0) {
            ...
            // 💡 4番目の引数（iouThreshold）に、スライダーと連動する変数「iouThreshold」をセット
    ...
    // 💡 重なり（IoU）スライダーのイベントリスナー（リアルタイムで変数反映＆枠を再計算）
    if (iouSlider) {
        iouSlider.addEventListener('input', (evt) => {
            iouThreshold = parseFloat(evt.target.value);
            if (iouValue) iouValue.textContent = iouThreshold.toFixed(2);
            refreshBBoxes();
        });
    }
